...
import { cancelCreditNote } from "@services/CreditNoteService";
...
export const CancelCreditNote = ({
    ...
    const initData = async (id: number) => {
        try {
            ...
            setData({
                ...
                createdAt: dayjs(res?.value.canceldAt),
    ...
    const onSubmitUpdateForm = async (values: any) => {
        let id = data.id;
        try {
            const res: any = await cancelCreditNote({ id: id });
            if (!res.errorCode) {
                toast.success(`${t("common:cancelCreditNote:success")}`);
                if (reloadTable) reloadTable();
            }
        } catch (e) {
            toast.error(`${t("common:cancelCreditNote:fail")}`);
        }
    }
    ...
    return (<div>
        <Modal title={t("menu:errorAccountingTransfer:cancelCreditNote")}
            open={isOpen}
